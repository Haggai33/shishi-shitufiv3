...
import { Plus, Calendar, Clock, MapPin, Users, ChefHat, Search } from 'lucide-react';
...
  const { events, menuItems, assignments, isLoading, user, isAdmin } = useStore();
...
                  {statusOptions.map(option => (<FilterButton key={option.value} label={option.label} count={filterCounts.status[option.value as keyof typeof filterCounts.status]} isActive={statusFilter === option.value} onClick={() => setStatusFilter(option.value as any)}/>))}
