...
export function EventDetails({ event, onBack }: EventDetailsProps) {
  ...
  // Wait for data to be loaded before allowing operations
  useEffect(() => {
    const timer = setTimeout(() => {
      setDataLoaded(true);
    }, 3000); // Give Firebase more time to load data
  ...
  // ניתן לשבץ כל עוד התאריך לא עבר (גם אם השעה עברה) והאירוע פעיל
  ...
  const handleAssignItem = (item: MenuItem) => {
    ...
    // Always allow assignment - the modal will handle name collection if needed
  ...
  const findUserAssignment = async (item: MenuItem): Promise<Assignment | null> => {
    ...
    console.log('Looking for assignment:', {
      itemId: item.id,
      userId: user.id,
      eventAssignments: eventAssignments.length,
      allAssignments: assignments.length
    });

    // First try local state
    let assignment = eventAssignments.find(a => 
      a.menuItemId === item.id && a.userId === user.id
    );

    if (assignment) {
      console.log('Found assignment in local state:', assignment);
      return assignment;
    }

    // If not found locally, try all assignments (in case of sync issues)
    assignment = assignments.find(a => 
      a.menuItemId === item.id && a.userId === user.id
    );

    if (assignment) {
      console.log('Found assignment in global state:', assignment);
      return assignment;
    }
  ...
  const handleEditAssignment = async (item: MenuItem) => {
    if (!user || !canAssign || !dataLoaded) {
      if (!dataLoaded) {
        toast.error('המערכת עדיין טוענת נתונים, אנא המתן...');
      }
      return;
    }
    ...
    if (assignment) {
      console.log('Found assignment for editing:', assignment);
      setEditingAssignment({ item, assignment });
    } else {
      console.log('No assignment found for editing');
      toast.error('לא נמצא שיבוץ לעריכה');
    }
  };
  ...
  const handleForceRefresh = async () => {
    ...
    try {
      ...
      // Wait a bit for the data to sync
      setTimeout(() => {
  ...
  console.log('EventDetails render:', {
    ...
    eventAssignments: eventAssignments.length,
    menuItems: eventMenuItems.length,
    dataLoaded
  });

  // Show bulk manager if requested
  if (showBulkManager) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-white rounded-xl shadow-md p-6 mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
          ...
          <div className="mt-2 text-xs text-blue-600">
            {isPast 
              ? 'האירוע הסתיים - ניתן לצפות בפרטים בלבד'
              : isFinished 
                ? 'האירוע בעיצומו - ניתן עדיין לשבץ פריטים'
                : 'האירוע פעיל - ניתן לשבץ פריטים'
            }
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <div className="flex items-center text-gray-600">
            <Calendar className="h-5 w-5 ml-3 text-orange-500" />
          ...
          <div className="flex items-center text-gray-600">
            <MapPin className="h-5 w-5 ml-3 text-orange-500" />
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <span className="text-blue-700 text-sm">טוען נתונים מהשרת ומסנכרן...</span>
            </div>
            <button
              onClick={handleForceRefresh}
              disabled={isRefreshing}
              className="flex items-center space-x-2 rtl:space-x-reverse px-3 py-1 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-lg text-sm transition-colors disabled:opacity-50"
            >
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
            ...
            <h2 className="text-lg font-semibold text-gray-900">
              ...
              {filteredItems.length > 0 && (
                <span className="text-gray-500 text-sm mr-2">
                  ({filteredItems.filter(item => item.assignedTo).length}/{filteredItems.length} משובצים)
                </span>
              )}
            </h2>
          </div>

          {/* Admin Bulk Management Button */}
          {isAdmin && (
            <button
              onClick={() => setShowBulkManager(true)}
              className="flex items-center space-x-2 rtl:space-x-reverse px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm transition-colors"
            >
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500">עדיין לא נוספו פריטים לקטגוריה הנבחרת</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {filteredItems.map((item) => (
              <MenuItemCard
                key={item.id}
                item={item}
                canAssign={canAssign && dataLoaded}
      ...
      {/* User Info Modal - Keep for backward compatibility */}
      {showUserInfo && (
        <UserInfoModal
